...
//PC实例化
	PC U_PC(.clk(clk), .rst(rst), .PCWr(pcWrite), .NPC(extDataOut), .PC(pcOut), .JUMP(jump), .JUMPAdr(jumpAdr));
...
//im指令寄存器实例化
	...
	assign jumpAdr = opCode[25:0];
	assign rfReadAdr1 = opCode[25:21];
	assign rfReadAdr2 = opCode[20:16];
	assign rfWriteAdr = (RegDst==0)?opCode[20:16]:opCode[15:11];
...
//RF寄存器堆实例化
	RF U_RF(.A1(rfReadAdr1), .A2(rfReadAdr2), .A3(rfWriteAdr), .WD(rfDataIn)
			, .clk(clk), .RFWr(RegWriteEn), .RD1(rfDataOut1), .RD2(rfDataOut2), .ReadEnable(rfReadEn));
